...
class Depth_Image(Node):
    def __init__(self):
        ...
        #self.turning = 0.0
        self.turning_elapsed_time = 6.25
        self.turning_pass_time = 7.75
    ...
    def status2driving(self, img):
        ...
        if len(inclinations) >= 2:
            ...
            if self.state == 'IDLE':
                ...
                cmd.linear.x = 2.4 - 2.5 * math.pow(abs(steering),1)
                ...
                #cmd.angular.z = -0.25
                cmd.angular.z = steering - kp_g * abs(math.pow((18.5 - self.dist_green), 3))
            elif self.state == 'PASS_GREEN':
                cmd.linear.x = 0.8
                cmd.angular.z = steering

            cmd.linear.x = np.clip(cmd.linear.x, -10000, 10000)
