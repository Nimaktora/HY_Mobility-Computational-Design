...
class Depth_Image(Node):
    def __init__(self):
        ...
        #self.turning = 0.0
        self.turning_elapsed_time = 3.5
        self.turning_pass_time = 4.5
    ...
    # circle -> light on/off status
    def get_circles2status(self, img, edges, dist_lim):
        ...
        if circles is not None:
          ...
          distance = img[center_y, center_x]
    ...
    def status2driving(self, img):
        ...
        if len(inclinations) >= 2:
            ...
            steering = -(kp * self.error + ki * self.ierror + kd * self.derror + kp_g * math.pow(self.duration_green, 2))
            ...
            if self.state == 'IDLE':
                ...
                cmd.linear.x = 6.5 - 32.5 * math.pow(abs(steering),2)
                ...
                cmd.linear.x = 3.7 - 0.8 * math.pow(abs(steering),3)
                #steer_base = steering - 0.05
                #steer_boost = -0.2 * pow(self.duration_green, 0.7)
                #cmd.angular.z = steer_base + steer_boost
            ...
            cmd.linear.x = np.clip(cmd.linear.x, -1.5, 10000)
    ...
    def depth_callback_sync(self, rgb_msg, depth_msg):
        ...
        self.status_red = self.get_circles2status(depth_raw_img, edges_red, 15)
        
        ## green
        green_mask = self.get_color_mask(depth_rgb_img, 'green', 30)
        edges_green = self.get_edge_line(green_mask)
        self.status_green = self.get_circles2status(depth_raw_img, edges_green, 18)
    ...
    def depth_callback(self, depth_msg):
        ...
        # print(type(depth_image)) # np.darray
        # print(depth_image.dtype) # float32 -> int8 (0~255)
        # print("SHape:", depth_image.shape)
        # print("Depth:", depth_image[360,640])
        # print("Depth:", depth_image[0,640])
        

        # # print(type(cv_rgb_image)) # np.darray
        # # cv_rgb_image = cv2.normalize(cv_rgb_image, None, 0, 255, norm_type=cv2.NORM_MINMAX)
        # depth_normal = cv2.normalize(depth_image, None, 0, 255, cv2.NORM_MINMAX)
        # # print((dst.dtype)) # float32
        # depth_normal = depth_normal.astype(np.uint8)  # float32 -> unit8
        # print(depth_normal.dtype)
        # cv2.imshow("Depth", depth_normal)
        # cv2.waitKey(10)
