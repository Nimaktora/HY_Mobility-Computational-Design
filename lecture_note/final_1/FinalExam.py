...
class Depth_Image(Node):
    def __init__(self):
        ...
        #self.turning = 0.0
        self.turning_elapsed_time = 8.5
        self.turning_pass_time = 12.5
    ...
    def status2driving(self, img):
        ...
        if len(inclinations) >= 2:
            ...
            if self.state == 'IDLE':
                ...
                cmd.linear.x = 1.75 -6.3 * math.pow(abs(steering),2)
                cmdlinearx_f = alpha * cmd.linear.x + (1.0-alpha) * cmdlinearx_f #low pass filter
                if cmdlinearx_f < 0:
                    cmd.linear.x = np.clip(cmdlinearx_f, -1.75, -1.74)
                elif cmdlinearx_f >= 0:
                    cmd.linear.x = np.clip(cmdlinearx_f, 1.0, 10000)
                cmd.angular.z = steering -0.008* math.pow(self.duration_green,2) - kp_g * math.pow((21.0 - self.dist_green), 3)
            elif self.state == 'PASS_GREEN':
                cmd.linear.x = 0.7
                cmd.angular.z = steering

            cmd.linear.x = np.clip(cmd.linear.x, -2.0, 10000)
    ...
    def depth_callback_sync(self, rgb_msg, depth_msg):
        ...
        self.status_green = self.get_distance2status(self.dist_green, 10)
